...
export default function BookingTimer() {
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      const diff = Math.floor((parseInt(stored, 10) - Date.now()) / 1000);
      if (diff <= 0) { setRemaining(0); setExpired(true); clearInterval(intervalRef.current); }
  ...
  const handleBack = () => { sessionStorage.removeItem(STORAGE_KEY); navigate('/reservar'); };
      ...
      {expired && (
        <div className="fixed inset-0 z-[9999] bg-black/70 flex items-center justify-center p-4">
          <div className="bg-card rounded-2xl shadow-2xl max-w-md w-full p-8 text-center">
            ...
            <p className="text-muted-foreground mb-6">{t('booking.timer_expired_desc')}</p>
