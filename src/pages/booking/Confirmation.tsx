    ...
    <PublicLayout>
      <div className="pt-20 section-padding min-h-screen bg-accent flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <p className="text-muted-foreground mb-8">{t('booking.confirmed_email')}</p>
          <div className="flex flex-col gap-3">
            <Link to={lp('/mis-reservas')} className="border-2 border-primary text-primary font-bold py-3 rounded-lg hover:bg-primary hover:text-primary-foreground transition-colors">
              {t('booking.view_reservation')}
            </Link>
            <Link to={lp('/')} className="bg-cta text-cta-foreground font-bold py-3 rounded-lg hover:opacity-90 transition-opacity">
              {t('back_home')}
            </Link>
          </div>
        </div>
